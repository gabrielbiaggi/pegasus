...
TICK_COUNT=100; CUSUM_MAX=5.0; HURST_MIN=0.45; CALM_THRESH=1.5e-6
...
balance = START_BAL; all_results = []; total_calls = 0
t0 = time.time()

print(f"{'Data':<12} {'Trades':>7} {'WR':>6} {'P&L':>8} {'Pico%':>7} {'Saldo':>8}  Parada")
print("-" * 68)
...
for day in days:
    ...
    while i < len(day_df) - MAX_HOLD - 1:
        ...
        for j in range(1, MAX_HOLD + 1):
            ...
            if cv - stake >= stake * TP_PCT:
                ...
                is_win = True; hold = j; break

        if is_win:
            wins += 1; sor_cd = 0
            if sor_step < SOROS_STEPS:
                sor_step += 1; sor_prof = round(sor_prof + profit, 2)
            else:
                sor_step = 0; sor_prof = 0.0
        else:
            losses += 1; sor_step = 0; sor_prof = 0.0; sor_cd = SOROS_COOLDOWN

        bal  = round(bal + profit, 2); peak = max(peak, bal)
        pnl  = bal - sod
    ...
    pnl_d = bal - sod; pk_p = (peak - sod) / sod * 100
    elapsed = time.time() - t0

    if stop_reason == 'DOBROU':
        flag = '🎯'; tag = f'DOBROU! ${bal:.2f}'
    elif stop_reason == 'TRAILING':
        flag = '🔒'; tag = f'+{pnl_d/sod*100:.0f}% trailing'
    elif pnl_d > 0:
        flag = '✅'; tag = f'+${pnl_d:.2f}'
    else:
        flag = '❌'; tag = f'-${abs(pnl_d):.2f}'

    print(f"{flag} {day}  {total:>6}  {wr:>5.1f}%  {pnl_d:>+7.2f}  {pk_p:>+6.1f}%  ${bal:>7.2f}  {tag}  [{elapsed:.0f}s]")
    sys.stdout.flush()

    all_results.append({'date': day, 'pnl': pnl_d, 'balance': bal, 'peak_pct': pk_p,
                        'wr': wr, 'total': total, 'wins': wins, 'losses': losses,
                        'doubled': stop_reason=='DOBROU', 'trailing': stop_reason=='TRAILING'})
    balance = bal

n = len(all_results)
if n == 0:
    print("Sem resultados."); sys.exit(0)
...
""")
print("  " + ("✅ SIM DOBRA" if nd/n>=0.2 else "⚠️  EM DIAS BONS SIM" if nd/n>=0.1 else "❌ NAO CONSISTENTE"))
