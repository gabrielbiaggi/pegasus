...
def api_backtest(response: Response):
    """Backtest estatístico rápido (Monte Carlo 2000 paths x 14 dias).
    Retorna resultados em < 3 segundos usando estatísticas reais do bot live.
    """
    response.headers["Cache-Control"] = "no-store"
    import numpy as np
    ...
    def sim_day(wr, bal):
        ...
        for _ in range(500):
            if sc > 0:
                ...
                stake = round(min(STAKE + sp, MAX_STAKE), 2)
            else:
                stake = STAKE
            stake = max(0.35, min(stake, bal))
            if stake < 0.35:
                break

            is_win = np.random.random() < wr
            if is_win:
                profit = round(stake * TP, 2)
                sc = 0
                if ss < SOROS:
                    ss += 1
                    sp = round(sp + profit, 2)
                else:
                    ss = 0
                    sp = 0.0
                wins += 1
            ...
            if pnl >= sod * SG:
                stop = "DOBROU"
                break
            if not trail and pnl >= sod * TS:
                trail = True
            if trail and pnl <= sod * TL:
                stop = "TRAILING"
                break
        ...
        return {
            ...
            "peak_pct": round((peak - sod) / sod * 100, 1) if sod > 0 else 0,
    ...
    # Simula N_PATHS × N_DAYS
    paths_final = []
    doubled_per_day = [0] * N_DAYS

    for _ in range(N_PATHS):
        bal = START
        for d in range(N_DAYS):
            sc_idx = np.random.choice(3, p=SC_PROBS)
            wr_mean, wr_std = WR_SCENARIOS[sc_idx]
            wr = float(np.clip(np.random.normal(wr_mean, wr_std), 0.55, 0.95))
            r = sim_day(wr, bal)
            bal = r["balance"]
            if r["doubled"]:
                doubled_per_day[d] += 1
        paths_final.append(bal)

    fb = np.array(paths_final)

    # Estatísticas por cenário (1 dia)
    scenario_stats = []
    for (wr_mean, wr_std), sc_name in zip(
        WR_SCENARIOS, ["Bom (83%)", "Neutro (79%)", "Ruim (70%)"]
    ):
        n_d = n_t = 0
        pnls = []
        for _ in range(1000):
            wr = float(np.clip(np.random.normal(wr_mean, wr_std * 0.5), 0.55, 0.95))
            r = sim_day(wr, START)
            pnls.append(r["pnl"])
            if r["doubled"]:
                n_d += 1
        ...
        scenario_stats.append(
            {
                "name": sc_name,
                "p_double": round(n_d / 10, 1),
                "p_trailing": round(n_t / 10, 1),
    ...
    return {
        ...
        "real_summary": {
            "days": len(real_results),
            "doubled": sum(1 for r in real_results if r["doubled"]),
            "positive": sum(1 for r in real_results if r["pnl"] > 0),
            "avg_wr": round(sum(r["wr"] for r in real_results) / len(real_results), 1),
        },
        "mc_summary": {
            "n_paths": N_PATHS,
            "n_days": N_DAYS,
            ...
            "p_doubled_ever": round(
                sum(1 for b in fb if b >= START * 2) / N_PATHS * 100, 1
            ),
        },
